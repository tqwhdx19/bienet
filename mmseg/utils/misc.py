...
def stack_batch(inputs: List[torch.Tensor],
    ...
    for i in range(len(inputs)):
        ...
        if data_samples is not None:
            ...
            # image augmentation
            if 'img_aug' in data_sample:
                img_aug = data_sample.img_aug
                img_aug = img_aug/255
                del data_sample.img_aug
                data_sample.img_aug = F.pad(img_aug, padding_size, value=pad_val)

            # image augmentation
            if 'original_img' in data_sample:
                original_img = data_sample.original_img
                original_img = original_img / 255
                del data_sample.original_img
                data_sample.original_img = F.pad(original_img, padding_size, value=pad_val)
